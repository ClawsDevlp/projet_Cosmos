...
$stmtCheckBadges = MyPDO::getInstance()->prepare(<<<SQL
    ...
    WHERE b.id_badge =
        ...
        WHERE p.id_joueur = :id_player AND b.id_badge = 16 AND t.nb_end IS NOT NULL
    ...
    OR b.id_badge =
        ...
        FROM partie p, 
            badges b LEFT OUTER JOIN badgesobtenus bo ON b.id_badge = bo.id_badge
        WHERE p.id_joueur = :id_player AND b.id_badge = 19 AND (SELECT COUNT(bo.id_badge) FROM badgesobtenus bo WHERE bo.id_joueur = :id_player GROUP BY bo.id_joueur) >= 18
        GROUP BY p.id_joueur)
    OR b.id_badge =
        (SELECT DISTINCT b.id_badge
        FROM joueur j, 
            badges b LEFT OUTER JOIN badgesobtenus bo ON b.id_badge = bo.id_badge
        WHERE j.id_joueur = :id_player AND b.id_badge = 20 AND LOWER(j.pseudo) LIKE "%pascale%")
     GROUP BY bo.id_badge
     HAVING SUM(CASE bo.id_joueur WHEN :id_player THEN 1 ELSE 0 END)=0;
SQL
    );
